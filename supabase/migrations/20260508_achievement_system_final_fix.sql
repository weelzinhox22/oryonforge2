-- Fix and Expand Achievement System Logic
-- Migration: 20260508_achievement_system_final_fix.sql
...
BEGIN
    -- 1. Base stats from activity_logs
    ...
    -- 6. App Usage Stats (Logins / Opens)
    current_logins := 0;
    current_app_opens := 0;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_logins') THEN
        SELECT COUNT(*) INTO current_app_opens FROM user_logins WHERE user_id = u_id;
        
        -- Logins (App opens WITHOUT registering activity)
        -- We calculate this as: total logins - total activity logs
        -- Actually, the user says "Abri o app 30 vezes sem registrar atividade"
        -- So we need to count logins on days where no activity was logged? 
        -- Or just total logins - total logs? 
        -- Let's use total logins where no activity exists for that login entry? 
        -- Usually it's simpler: total logins - count(logs). 
        -- But for "Só Observando", it implies 30 opens that didn't lead to a log.
        current_logins := GREATEST(0, current_app_opens - current_workouts);
    END IF;
    ...
    FOR ach_record IN SELECT * FROM achievements WHERE category = 'individual' LOOP
        ...
        CASE ach_record.requirement_type
            ...
            WHEN 'opens' THEN is_met := current_app_opens >= ach_record.requirement_value;
            WHEN 'logins' THEN is_met := current_logins >= ach_record.requirement_value;
            WHEN 'daily_logs' THEN 
                SELECT COUNT(*) >= ach_record.requirement_value INTO is_met 
                FROM activity_logs 
                WHERE user_id = u_id AND (created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo')::DATE = (log_created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo')::DATE;
            WHEN 'specific_workout' THEN
                SELECT COUNT(*) >= ach_record.requirement_value INTO is_met
                FROM activity_logs
                WHERE user_id = u_id AND (activity_type NOT ILIKE '%corrida%' AND activity_type NOT ILIKE '%bike%' AND activity_type NOT ILIKE '%esteira%');
            ...
            WHEN 'weather_rain' THEN
                SELECT COUNT(*) >= ach_record.requirement_value INTO is_met
                FROM activity_logs
                WHERE user_id = u_id AND weather_status = 'rain';
            WHEN 'comeback' THEN
...
-- Retroactive update for all users
DO $$
DECLARE
    log_record RECORD;
BEGIN
