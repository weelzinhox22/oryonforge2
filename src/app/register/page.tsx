...
import { Eye, EyeOff, ChevronLeft, ArrowRight, CheckCircle2, User, Mail, Lock, LayoutGrid, AlignLeft } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const [dashboardStyle, setDashboardStyle] = useState<'premium' | 'minimalist'>('premium');
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (username.length < 3) {
      setToast({ isVisible: true, message: 'O nome deve ter pelo menos 3 caracteres.', type: 'error' });
      return;
    }

    if (password.length < 6) {
      setToast({ isVisible: true, message: 'A senha precisa ter no mínimo 6 caracteres.', type: 'error' });
      return;
    }
      ...
      <div className="hidden md:flex w-1/2 relative items-center justify-center p-12">
        ...
        >
          ...
          <h2 className="text-4xl font-black tracking-tight mb-4 leading-tight">
            Comece sua <br/><span className="text-[#CCCC00]">Jornada.</span>
          </h2>
          <p className="text-[#606070] text-lg leading-relaxed">
            Crie sua conta para acessar o ecossistema, registrar resultados e participar de grupos na plataforma.
          </p>
        </motion.div>
      </div>
          ...
          {!isSuccess ? (
            ...
            >
              <motion.div variants={itemVariants} className="md:hidden flex flex-col items-center text-center mb-10">
                ...
                <p className="text-[#606070] text-sm">Insira seus dados para começar.</p>
              </motion.div>

              <motion.div variants={itemVariants} className="hidden md:block mb-10">
                <h1 className="text-4xl font-black tracking-tight mb-2">Registro</h1>
                <p className="text-[#606070]">Preencha os campos abaixo para criar seu perfil.</p>
              </motion.div>

              <form onSubmit={handleSignUp} className="space-y-5">
                <motion.div variants={itemVariants} className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                    <User className={`w-5 h-5 transition-colors duration-300 ${focusedField === 'username' || username ? 'text-[#CCCC00]' : 'text-[#606070]'}`} />
                  </div>
                  <input
                    type="text"
                    value={username}
                ...
                <motion.div variants={itemVariants} className="space-y-3 pb-2">
                  <label className="text-[10px] font-black text-[#303035] uppercase tracking-[0.2em] px-2">Visual do Dashboard</label>
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      { id: 'premium', icon: LayoutGrid, label: 'Premium' },
                      { id: 'minimalist', icon: AlignLeft, label: 'Minimal' },
                    ].map((style) => (
                      <button
                        key={style.id}
                        type="button"
                        onClick={() => setDashboardStyle(style.id as any)}
                        className={`flex items-center gap-3 p-4 rounded-2xl border transition-all duration-300 ${
                          dashboardStyle === style.id 
                            ? 'bg-[#CCCC00]/5 border-[#CCCC00]/30 text-[#CCCC00]' 
                            : 'bg-white/[0.02] border-white/10 text-[#606070] hover:border-white/20'
                        }`}
                      >
                        <style.icon size={16} />
                ...
                <motion.div variants={itemVariants} className="pt-2">
                  ...
                  >
                    ...
                    ) : <><ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" /> Criar Perfil</>}
                  </button>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="text-center pt-6">
                  <p className="text-[#606070] text-sm">
                    Já possui conta?{' '}
                    <Link href="/login" className="text-white font-semibold hover:text-[#CCCC00] transition-colors">
                      Acessar Agora
                    </Link>
                  </p>
                </motion.div>
            ...
            <motion.div 
              ...
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              className="w-full max-w-sm relative z-10 flex flex-col items-center text-center p-10 bg-white/[0.02] border border-white/10 rounded-[2rem] backdrop-blur-xl"
            >
              <motion.div 
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                transition={{ type: "spring", stiffness: 200, delay: 0.2 }}
                className="w-20 h-20 bg-[#CCCC00]/10 rounded-2xl flex items-center justify-center mb-6 border border-[#CCCC00]/20"
              >
                <CheckCircle2 className="w-10 h-10 text-[#CCCC00]" />
              </motion.div>
              <h2 className="text-3xl font-black tracking-tight mb-2">Conta Criada</h2>
              <p className="text-[#606070] mb-8">Estamos preparando a sua plataforma.</p>
